...
class Agent:
    def __init__(
        ...
        # 打印调试信息，确保设备和 GPU 配置正确
        print(f"GPU Index: {args.gpu}")  # 打印 GPU 索引
        print(f"Device: {args.device}")  # 打印设备（cpu/cuda）
    ...
    def load(self, file_name):
        ckpt = torch.load(file_name, map_location=self.args.device)
...
def main():
    ...
    agent = Agent(args, additional_special_tokens, emotion_statistic, kw_vocab)
    #for save model
    save_path = args.save_model_path + "PERL-CAusion.ckpt"
    # for save pretrain model
    save_pretrain_path = args.save_model_path + "pretrain_PERL-CAusion.ckpt"
    # logger
